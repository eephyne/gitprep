...
##		"rights not null default '2'"
	];
	#values of rights are: 
	#0-Banned user (not allowed to create repo or post issue/comment
	#1-Classic user (not allowed to create repo but can't post issue/comment
	#2-Power user (can create repo and post issue/comment
...
create table issues (
	...
	project not null default '',
	project_user not null,
	issue_id integer not null
);
EOS
		$dbi->execute($sql);
	};
#create issues_comments table
	eval {
		my $sql = <<"EOS";
...
sub issues {
	...
	# Issues 
	my $issues = $self->app->dbi->model('issues')->select(
		where => {project => $project,project_user => $user},
		append => 'order by update_date ASC'
	)->all;
	#SELECT * FROM `ttrss_feeds` WHERE id IN (SELECT MAX(id) FROM `ttrss_feeds` GROUP BY cat_id) 
	return $issues;
}
sub issue {
	my ($self,$user, $project, $issue_id) = @_;

	my $issue = $self->app->dbi->model('issues')->select(
		where => {issue_id => $issue_id,project => $project,project_user => $user},
	)->all;
	return $issue;
}
sub issue_content {
	...
	my $issue_comments = $self->app->dbi->model('issues_comments')->select(
		where => {issue_id => $issue_id,project =>$project,project_user => $user},
...
sub create_issue {
	my ($self, $user, $project_user, $project, $issue_title, $issue_content, $opts) = @_;
		$self->app->log->error("user $user, project $project, title $issue_title");
	my $next_issue_id = $self->app->dbi->model('issues')->count(
		where => {project => $project},
	)+1;
	
	$self->app->dbi->model('issues')->insert({user_id => $user, 
																					title => $issue_title, 
																					project => $project,
																					status => 'open',
																					project_user => $project_user,
																					issue_id => $next_issue_id});
	$self->app->dbi->model('issues_comments')->insert({user_id => $user, 
...
sub add_issue_content {
	my ($self, $user, $project, $issue_id , $issue_type, $issue_content, $opts) = @_;
	...
	$self->app->dbi->model('issues_comments')->insert(user_id => $user, 
																					...
																					type => $issue_type,
																					issue_id => $issue_id);
	$self->app->dbi->model('issues')->update(type => $issue_type,where  => {project => $project,issue_id => $issue_id});	
}
